...
LOGGING_LEVEL = logging.DEBUG # WARNING  # DEBUG to output all duplicate spans
...
# replacing if name == main
def get_heads(data_root, file_base):
    for root, dirs, files in os.walk(data_root):
        data_dirname =   root.split("/")[-1]
        print("get_heads",root,data_root,data_dirname)
        f_name = "{}_{}{}.jsonl"
        logging.basicConfig(level=LOGGING_LEVEL)
        w_path = os.path.join(root, f_name)
        for split in SPLITS:
            with jsonlines.open(w_path.format(file_base,split, ""), mode="r") as inf:
                with jsonlines.open(w_path.format(file_base, split, "_head"), mode="w") as outf:
                    deleted_spans = 0
                    deleted_clusters = 0
                    total_spans = 0
                    total_clusters = 0
    ...
    # Copy over the head files
    heads_dir = data_root+"_heads"
    os.makedirs(heads_dir)
    for root, dirs, files in os.walk(data_root):
        for filename in files:
            if "_head" in filename:
                shutil.copy(os.path.join(root, filename), os.path.join(heads_dir, filename))
